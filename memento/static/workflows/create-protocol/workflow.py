...
class TaskItem(BaseModel):
    ...
    body: str = Field(
        default="",
        description="Detailed description: rich text with lists, code blocks, examples",
    )
...
class Task(BaseModel):
    ...
    heading: str = Field(
        description="Concise, action-oriented title (plain text, one line)"
    )
    description: str = Field(
        default="", description="Context or explanation for this task group (rich text)"
    )
    subtasks: list[TaskItem] = Field(
        default_factory=list, description="Items to implement under this task"
    )
    acceptance_criteria: list[str] = Field(
        default_factory=list,
        description="2-5 concrete, verifiable statements about what this task must achieve",
    )
...
class StepDef(BaseModel):
    ...
    objective: str = Field(
        description="What this step accomplishes and why (rich text)"
    )
    tasks: list[Task] = Field(description="Task groups (each becomes a TDD unit)")
    constraints: list[str] = Field(
        default_factory=list,
        description="Acceptance criteria / DoD items (one per entry)",
    )
    impl_notes: str = Field(
        default="", description="Key files, patterns, approach (rich text)"
    )
    verification: list[str] = Field(
        default_factory=list, description="Shell commands to verify step works"
    )
    context_inline: str = Field(
        default="",
        description="Inline context notes: references to spec sections, architectural decisions, research summaries (rich text)",
    )
    context_files: list[str] = Field(
        default_factory=list, description="Reference file paths (Memory Bank, _context)"
    )
    starting_points: list[str] = Field(
        default_factory=list, description="Key source files to start from"
    )
    memory_bank_impact: list[str] = Field(
        default_factory=list, description="Memory Bank files to update after step"
    )
...
class GroupDef(BaseModel):
    ...
    title: str = Field(
        description="Group title (plain text, e.g., 'Infrastructure', 'Auth')"
    )
...
class ItemWrapper(BaseModel):
    ...
    type: Literal["step", "group"] = Field(
        description="'step' for a single step, 'group' for a directory of steps"
    )
    step: StepDef | None = Field(default=None, description="Present when type='step'")
    group: GroupDef | None = Field(
        default=None, description="Present when type='group'"
    )
...
class ProtocolPlan(BaseModel):
    ...
    decision: str = Field(
        description="Approach and key architectural choices (rich text)"
    )
    rationale: str = Field(
        description="Why this approach over alternatives (rich text)"
    )
    consequences_positive: list[str] = Field(
        description="Positive consequences (one per entry)"
    )
    consequences_negative: list[str] = Field(
        description="Negative consequences with mitigations (one per entry)"
    )
    items: list[ItemWrapper] = Field(
        description="Protocol items in execution order: steps and groups"
    )
...
class PrdOutput(BaseModel):
    ...
    constraints: list[str] = Field(
        description="Technical constraints, timeline, resources"
    )
    acceptance_criteria: list[str] = Field(
...
WORKFLOW = WorkflowDef(
    ...
    blocks=[
        ...
        ShellStep(
            ...
            condition=lambda ctx: (
                getattr(ctx.results.get("edit-plan"), "status", None) != "success"
                and getattr(ctx.results.get("plan-protocol"), "status", None)
                == "success"
            ),
        ),
