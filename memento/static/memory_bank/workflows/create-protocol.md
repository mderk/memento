...
**What belongs in `_context/`:** research notes, API docs excerpts, architecture decisions, rejected approaches with rationale, benchmarks, stakeholder feedback. Also `findings.md` — runtime discoveries promoted from step files during execution (see `/process-protocol` workflow).
...
-   `/process-protocol` workflow — Execute protocol steps
