...
Goal: make `memento-pi` usable for local development on real protocol workflows before release packaging/config work is done.

1. keep current dev bootstrap (`pi install -l ...`, local `MEMENTO_WORKFLOW_DIR` fallback)
2. add thin `/mw create-protocol` wrapper
3. add thin `/mw process-protocol` wrapper
4. add protocol-dir resolution helpers (number / path / description)
5. add `.last_run` resume/fresh prompt for `process-protocol`
6. keep `.claude/skills/**` and `.workflows/**` unchanged
...
P0a done when:
- dev can run `/mw create-protocol ...` against real local repo workflows
- dev can run `/mw process-protocol ...` against real local repo workflows
- `process-protocol` supports resume/fresh from `.last_run`
- no changes were required to `.claude/skills/**` or `.workflows/**`
...
| Area | Test |
|---|---|
| dev dogfood | `/mw create-protocol` integration on real workflow |
| dev dogfood | `/mw process-protocol` path resolution + `.last_run` resume prompt |
...
3. `memento-pi` protocol command helpers
4. `memento-pi` `/mw create-protocol`
5. `memento-pi` `/mw process-protocol`
6. `.last_run` resume/fresh prompt
7. `memento-pi` config migration to `.pi/settings.json`
8. `memento-pi` server bootstrap config
9. `memento-pi` `workflowDirs`
10. `memento-pi` `/wf resume`
11. `memento-pi` `/mw develop`
12. remaining wrapper commands
13. optional P2 `memento` environment wrappers
