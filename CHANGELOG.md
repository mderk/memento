...
- **Commit workflow**: `git commit` returning "nothing to commit" during split-commit no longer halts the workflow — returns `skipped` status instead of `error`, allowing remaining groups to proceed
